...
class LayerPanel(QWidget):
    ...
    def restore_settings(self):
        ...
        self.restoreGeometry(
            settings.value("editor/layer_panel/geometry", self.saveGeometry())
        )
    ...
    def setup_toolbar(self):
        ...
        shrink_action = self.toolbar.addAction(
            shrink_icon, "shrink palette", self.handle_shrink
        )
        enlarge_action = self.toolbar.addAction(
            enlarge_icon, "enlarge palette", self.handle_enlarge
        )
        delete_action = self.toolbar.addAction(
            delete_icon, "delete", self.handle_delete
        )
...
class LayerList(QScrollArea):
    ...
    def set_item_size(self, size):
        ...
        self.item_size = size
        for item in (
            self._items_layout.itemAt(i) for i in range(self._items_layout.count())
        ):
    ...
    def update_list(self):
        pool = [
            self._items_layout.takeAt(0).widget()
            for _ in range(self._items_layout.count())
        ]
...
class LayerListItem(QFrame):
    ...
    def __init__(self, *args):
        ...
        self._visibility_button.setIcon(nearest_icon(":/icons/layer_icons_eye_open"))
    ...
    def update_visibility_button(self):
        if not self.layer.hidden:
            ...
            self._visibility_button.setIcon(nearest_icon(":/icons/layer_icons_eye_closed"))
...
class LayerImageView(QWidget):
    ...
    def update_size(self):
        if self._image:
            new_size = (
                self._image.rect().size().scaled(self.max_size, Qt.KeepAspectRatio)
            )
